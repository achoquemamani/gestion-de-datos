...
BEGIN
	ALTER TABLE HARAKIRI.BI_HECHO_VENTA_DESCUENTO
	ADD CONSTRAINT FK_BI_HECHO_VENTA_DESCUENTO_BI_MEDIO_DE_PAGO FOREIGN KEY(MEDIO_DE_PAGO_CODIGO) REFERENCES HARAKIRI.BI_MEDIO_DE_PAGO(MEDIO_DE_PAGO_CODIGO)
	PRINT('Foreign key entre las tablas HARAKIRI.BI_HECHO_VENTA_DESCUENTO y HARAKIRI.BI_MEDIO_DE_PAGO agregada')
END
GO----------------------------------------------------------------------------------------------------------
/* MIGRACION DE TABLAS */
...
SELECT
	...
	SUM(HVP.PRODUCTO_PRECIO_TOTAL) - SUM(HCP.PRODUCTO_CANTIDAD*HCP.PRODUCTO_PRECIO_UNITARIO) - (
		SELECT SUM(VENTA_PERIODICIDAD.MEDIO_DE_PAGO_COSTO) FROM (
			SELECT 
				MDP.MEDIO_DE_PAGO_COSTO
			FROM HARAKIRI.BI_HECHO_VENTA_PRODUCTO VP
...
PRINT('Vista HARAKIRI.GANANCIA_POR_CANAL_DE_VENTA_POR_MES creada. Cant filas: ' + @CANTIDAD_REGISTROS)

---vista 2 -------------------------------------------------------------------------------------------------------------------------------------------

IF OBJECT_ID('HARAKIRI.TOP_5_PRODUCTOS_CON_MAYOR_RENTABILIDAD_ANUAL') IS NOT NULL
BEGIN
	DROP VIEW HARAKIRI.TOP_5_PRODUCTOS_CON_MAYOR_RENTABILIDAD_ANUAL
    PRINT('Vista HARAKIRI.TOP_5_PRODUCTOS_CON_MAYOR_RENTABILIDAD_ANUAL eliminada')
END
GO

CREATE VIEW HARAKIRI.TOP_5_PRODUCTOS_CON_MAYOR_RENTABILIDAD_ANUAL
AS
...
DECLARE @CANTIDAD_REGISTROS NVARCHAR(255)
SET @CANTIDAD_REGISTROS = (SELECT COUNT(*) FROM HARAKIRI.TOP_5_PRODUCTOS_CON_MAYOR_RENTABILIDAD_ANUAL)
PRINT('Vista HARAKIRI.TOP_5_PRODUCTOS_CON_MAYOR_RENTABILIDAD_ANUAL creada. Cant filas: ' + @CANTIDAD_REGISTROS)
...
PRINT('Vista HARAKIRI.TOP_5_CATEGORIAS_VENDIDAS_X_RANGO_ETARIO_CLIENTES_X_MES creada. Cant filas: ' + @CANTIDAD_REGISTROS)
...
AS
SELECT 
	SUM(VP.PRODUCTO_PRECIO_TOTAL) AS INGRESOS_TOTALES,
	SUM(VP.PRODUCTO_PRECIO_TOTAL)-CANT_ENVIO_MES_PROV_MDE*MDP.MEDIO_DE_PAGO_COSTO AS INGRESOS_TOTALES_MENOS_COSTO,
	T.TIEMPO_ANIO,
	T.TIEMPO_MES,
	MDP.MEDIO_DE_PAGO_NOMBRE,
	CANT_ENVIO_MES_PROV_MDE
FROM HARAKIRI.BI_HECHO_VENTA_PRODUCTO VP 
JOIN HARAKIRI.BI_MEDIO_DE_PAGO MDP ON MDP.MEDIO_DE_PAGO_CODIGO = VP.MEDIO_DE_PAGO_CODIGO
JOIN HARAKIRI.BI_TIEMPO T ON VP.TIEMPO_CODIGO = T.TIEMPO_CODIGO
--JOIN HARAKIRI.BI_HECHO_VENTA_DESCUENTO HVD ON HVD.TIEMPO_CODIGO = T.TIEMPO_CODIGO
--JOIN HARAKIRI.BI_VENTA V ON V.VENTA_CODIGO = VP.VENTA_CODIGO
GROUP BY T.TIEMPO_ANIO , T.TIEMPO_MES,MDP.MEDIO_DE_PAGO_NOMBRE,VP.CANT_ENVIO_MES_PROV_MDE,MDP.MEDIO_DE_PAGO_COSTO 
GO

DECLARE @CANTIDAD_REGISTROS NVARCHAR(255)
SET @CANTIDAD_REGISTROS = (SELECT COUNT(*) FROM HARAKIRI.INGRESOS_TOTALES_X_MEDIODEPAGO_X_MES)
PRINT('Vista HARAKIRI.HARAKIRI.INGRESOS_TOTALES_X_MEDIODEPAGO_X_MES creada. Cant filas: ' + @CANTIDAD_REGISTROS)
...
PRINT('Vista HARAKIRI.IMPORTE_TOTAL_DESCUENTOS_SEGUN_TIPO_DE_DESCUENTO_X_CANAL_VENTA_X_MES creada. Cant filas: ' + @CANTIDAD_REGISTROS)
...
PRINT('Vista HARAKIRI.PORCENTAJE_DE_ENVIOS_PROVINCIA_X_MES creada. Cant filas: ' + @CANTIDAD_REGISTROS)
...
SELECT 
	...
	(
		SELECT AVG(ENVIOS.ENVIO_PRECIO_MES_PROV_MDE) FROM (
			SELECT
				DISTINCT HVP.TIEMPO_CODIGO, HVP.PROVINCIA_CODIGO, HVP.MEDIO_ENVIO_CODIGO, ENVIO_PRECIO_MES_PROV_MDE
			FROM HARAKIRI.BI_HECHO_VENTA_PRODUCTO HVP
			INNER JOIN HARAKIRI.BI_TIEMPO BI_T ON BI_T.TIEMPO_CODIGO = HVP.TIEMPO_CODIGO
			INNER JOIN HARAKIRI.BI_PROVINCIA BI_P ON BI_P.PROVINCIA_CODIGO = HVP.PROVINCIA_CODIGO
			INNER JOIN HARAKIRI.BI_MEDIO_DE_ENVIO BI_MDE ON BI_MDE.MEDIO_ENVIO_CODIGO = HVP.MEDIO_ENVIO_CODIGO
			WHERE 
			BI_T.TIEMPO_ANIO= T.TIEMPO_ANIO AND
			BI_P.PROVINCIA_NOMBRE = P.PROVINCIA_NOMBRE AND
			BI_MDE.MEDIO_ENVIO_NOMBRE= MDE.MEDIO_ENVIO_NOMBRE
		) ENVIOS
...
PRINT('Vista HARAKIRI.PROMEDIO_DE_ENVIO_PROVINCIA_X_MEDIO_DE_ENVIO_ANUAL creada. Cant filas: ' + @CANTIDAD_REGISTROS)
...
PRINT('Vista HARAKIRI.AUMENTO_DE_PRECIOS_X_PROVEEDOR_ANUAL creada. Cant filas: ' + @CANTIDAD_REGISTROS)
...
PRINT('Vista HARAKIRI.TOP_3_PRODUCTOS_CON_MAYOR_REPOSICION_ANUAL creada. Cant filas: ' + @CANTIDAD_REGISTROS)