...
----VISTA 8 ---------------
/*
CREATE VIEW HARAKIRI.AUMENTO_PROMEDIO_PRECIOS_PROVEEDOR_X_ANIO
AS


SELECT  T.TIEMPO_ANIO,CP.PROVEEDOR_CUIT
,(
				SELECT MAX(CP2.PRODUCTO_PRECIO_UNITARIO)
				
                from HARAKIRI.BI_HECHO_COMPRA_PRODUCTO CP2
					INNER JOIN HARAKIRI.BI_TIEMPO AS T2 ON (CP2.TIEMPO_CODIGO = T2.TIEMPO_CODIGO)
                   		INNER JOIN HARAKIRI.BI_PRODUCTO AS P ON (CP2.PRODUCTO_CODIGO = P.PRODUCTO_CODIGO) 

				--WHERE C.TIPO_COMPRA_VENTA = 'COMPRA'
				AND CP2.PROVEEDOR_CUIT = CP.PROVEEDOR_CUIT
				AND T2.TIEMPO_ANIO = T.TIEMPO_ANIO
			
			) AS XXXXXXX,

--,C.CANAL_NOMBRE,VD.TIPO_DESCUENTO_CODIGO,SUM(VD.DESCUENTO_IMPORTE) AS [Importe Total]
FROM HARAKIRI.BI_HECHO_COMPRA_PRODUCTO CP 
JOIN HARAKIRI.BI_PROVEEDOR P ON P.PROVEEDOR_CUIT = CP.PROVEEDOR_CUIT
JOIN HARAKIRI.BI_TIEMPO T ON CP.TIEMPO_CODIGO = T.TIEMPO_CODIGO 
--JOIN HARAKIRI.BI_TIPO_DE_DESCUENTO TD ON VD.TIPO_DESCUENTO_CODIGO = TD.TIPO_DESCUENTO_CODIGO

--TODO

GROUP BY  T.TIEMPO_ANIO,P.PROVEEDOR_CUIT
GO


DECLARE @CANTIDAD_REGISTROS NVARCHAR(255)
SET @CANTIDAD_REGISTROS = (SELECT COUNT(*) FROM HARAKIRI.AUMENTO_PROMEDIO_PRECIOS_PROVEEDOR_X_ANIO)
PRINT('View AUMENTO_PROMEDIO_PRECIOS_PROVEEDOR_X_ANIO creada. Cant filas: ' + @CANTIDAD_REGISTROS)
*/
---vista4 --------
...
AS
SELECT SUM(V.VENTA_TOTAL-MDP.MEDIO_DE_PAGO_COSTO) AS INGRESOS_MEDIOS , T.TIEMPO_ANIO , T.TIEMPO_MES ,MDP.MEDIO_DE_PAGO_NOMBRE
FROM HARAKIRI.BI_HECHO_VENTA_PRODUCTO VP 
